...
The second graph for midfielders highlights a linear correlation between the number of failed tackles and number of fouls. The midfielders that committed a higher number of failed tackles were typically assigned more fouls by the referees. Accordingly, the points that have the greatest positive distance from the linear regression line represent the most dangerous midfielders. This is because the outliers 
